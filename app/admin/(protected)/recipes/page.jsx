...
export default function RecipesPage() {
    ...
    const handleDelete = async (id) => {
        if (confirm('Are you sure you want to delete this recipe?')) {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/recipes/${id}`, {
                    method: 'DELETE',
                    headers: {
    ...
    return (
        <main className="mx-2 lg:mx-10 2xl:mx-20">
            ...
            <div className="flex mt-8 justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h2 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-[#8b5cf6]">Recipes</h2>
                    <p className="text-sm text-gray-400 mt-1">Manage your delicious meal instructions</p>
                </div>
                <Link href={'/admin/recipes/create'} className="bg-primary text-white px-6 py-3 rounded-xl font-bold flex items-center hover:scale-105 transition-all shadow-lg shadow-primary/20">
            ...
            <section className="mb-20">
                {isLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {[...new Array(8)].map((_, i) => <RecipeListingSkeleton key={i} />)}
                    </div>
                ) : error ? (
                    ...
                    <div className="bg-white p-20 rounded-xl text-center border border-dashed border-gray-200">
                        <span className="icon-[arcticons--reciper] text-gray-200 w-24 h-24 mb-4 mx-auto block" />
